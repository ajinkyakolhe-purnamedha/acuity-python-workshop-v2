...
> Applies Module 3's `BankAccount` concepts to `Product`. The big refactor: Lab 1/2's product-**dicts** and loose **functions** become a typed **`@dataclass`** and a **`ProductCatalog` class** whose methods are those functions. (Decorators are a separate module — here you just *use* `@dataclass` and `@app.get`.)
