...
mock `requests.Session`. By default it talks to a LOCAL Ollama server
(no API key); set OPENAI_API_KEY to use real OpenAI instead.
"""
...
# no cost. The agent/registry/loop below are unchanged -- we only point the SAME
# OpenAI SDK at a different base_url. That IS the injection seam this lab is about.
